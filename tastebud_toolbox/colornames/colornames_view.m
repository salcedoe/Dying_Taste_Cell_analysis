...
if nargin<2
	...
	assert(ischar(order)&&isrow(order),'Second input <sort> must be a string.')
...
if isempty(fgh)||~ishghandle(fgh)
	...
	fgh = figure('HandleVisibility','callback', 'IntegerHandle','off',...
		...
		'Toolbar','figure', 'Units','pixels', 'ResizeFcn',@cnvResize);
	%
...
txf = @(s,b,c)text('Parent',axh, 'String',s, 'BackgroundColor',b,...
	...
	'Clipping','on','VerticalAlignment','bottom', 'HorizontalAlignment','right');
%
	function cnvTxtDraw()
...
%
	function cnvResize(~,~)
		...
		uiw = (fgp(3)-gap)/2-gap;
		set(pah, 'Position',[1*gap+0,  fgp(4)-top-gap,uiw,top])
		set(srh, 'Position',[2*gap+uiw,fgp(4)-top-gap,uiw,top])
